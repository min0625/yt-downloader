...
mise run check             # 所有檢查：格式化、lint fix、ty 型別檢查、測試
```
...
- **Windows**：console 模式打包；GUI 啟動時自動隱藏 console 視窗。
- **macOS**：可雙擊 `.app` 開啟 GUI；或在 terminal 帶參數進入 CLI 模式。

## Current Status

- ✅ **CLI 模式**：`--url`、`--mode`、`--format`、`--output-dir` 全部實作。
- ✅ **GUI 模式**：NiceGUI 3.x 原生視窗，含即時下載進度顯示。
...
- 測試：`uv run pytest` 目前收集 17 個測試並全部通過。
